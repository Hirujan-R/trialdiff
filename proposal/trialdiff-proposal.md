...
lineage_from_metadata(metadata, aliases = NULL, overrides = NULL)
lineage_review(x)
lineage_provenance(x)
td_output(id, depends_on, type = c("analysis", "output"))
output_registry(...)
...
* Analysis/output registry (`td_output()`, `output_registry()`, `add_outputs()`)
  and edge editing (`add_edges()`, `remove_edges()`) (implemented).
* Lineage import from `metacore` (implemented) with `aliases`/`overrides`, and
  from a tidy specification (CSV/YAML).
